(function () {
    var currentPage = 1,
        pageAll = $('div.slider3d__handle__item').length;
    var bBtn;
    var rotatingSlider = function (selector, options) {
        function initSingleSlider($el, options) {
            var $slider, $rotaters, $handle, $handleItems, numOfItems, angle, currentAngle = 0, prefix = '.slider3d__',
                handlePrefix = prefix + 'handle__', rotating = false;
            var defaultOptions = {
                speed: 1100,
                dragSpeedCoef: 0.7,
                handleSpeedCoef: 6,
            ...
            function rotateSlider(delta) {
                ...
                setTimeout(function () {
                    ...
                    var timer = setTimeout(function () {
                        $('.onepeo').eq(currentPage - 1).show().addClass('peo');
                        $('.jingzi').eq(currentPage - 1).show().addClass('suoxiao');
                        $('.qipao').eq(currentPage - 1).show().addClass('float');
                        $('.jinengbox').eq(currentPage - 1).addClass('unanimate');
                        $('.wuqibox').eq(currentPage - 1).addClass('unanimate');
                        $('.yuanhuabox').eq(currentPage - 1).addClass('unanimate');
                        $('.quancon').eq(currentPage - 1).addClass('unanimate');
            ...
            function scrollHandler(e) {
                ...
                $('.onepeo').hide().removeClass('peo').removeClass('on');
                $('.jingzi').hide().removeClass('suoxiao');
                $('.jingzi').removeClass('on');
                bBtn = true;
                $('.qipao').hide().removeClass('float');
                $('.jinengbox').removeClass('unanimate');
                $('.wuqibox').removeClass('unanimate');
                $('.yuanhuabox').removeClass('unanimate');
                $('.quancon').removeClass('unanimate');
            ...
            $('.jingzi').each(function (index, e) {
                bBtn = true;
                $(this).click(function () {
                    if (bBtn) {
                        $(this).addClass('on');
                        $('.onepeo').eq(index).addClass('on');
                        bBtn = false;
                    } else {
                        $(this).removeClass('on');
                        $('.onepeo').eq(index).removeClass('on');
                        bBtn = true;
                    }
                });
            })
            function initControls() {
                ...
                $(document).on('click', '.slider3d__control', function () {
                    ...
                    if ($(this).hasClass('m--up')) {
                        navigateUp();
                        $('.onepeo').hide().removeClass('peo');
                        $('.onepeo').hide().removeClass('on');
                        $('.jingzi').removeClass('on');
                        bBtn = true;
                        $('.qipao').hide().removeClass('float');
                        $('.jinengbox').removeClass('unanimate');
